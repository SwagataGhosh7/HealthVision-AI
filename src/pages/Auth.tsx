...
const Auth = () => {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (result) {
        // User logged in successfully (popup method on localhost)
        console.log("✅ Google sign-in successful!");
        toast.success("Welcome back!");
        navigate("/dashboard");
      } else {
        // Redirect method on production - user will be redirected away and back
        console.log("📍 Redirecting to Google... (Firebase Hosting)");
        // Don't show a toast here since the page will reload
      }
    } catch (error: any) {
    ...
    <div className="min-h-screen bg-gradient-hero flex items-center justify-center px-4">
      ...
      >
        <div className="text-center mb-8">
          ...
          <p className="text-muted-foreground text-sm">Make Healthcare Great Again</p>
        </div>
        ...
        <div className="bg-card border border-border/60 rounded-2xl p-8">
          ...
          <Button
            variant="outline"
            className="w-full mb-6 border-border/60 text-foreground hover:bg-secondary/50"
